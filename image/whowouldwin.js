...
module.exports = {
    config: {
        ...
        noalias: [''],
        category: "image",
        description: "Who would win amongst the two?",
    ...
    run: async (bot, message, args) => {
        ...
        if(!args[0]) return message.channel.send("**Enter Name Of The First Fighter!**")
        if(!args[1]) return message.channel.send("**Enter Name Of The Second Fighter!**")
        ...
        try {
            let res = await fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=whowouldwin&user1=${user.user.displayAvatarURL({ format: "png", size: 512 })}&user2=${user2.user.displayAvatarURL({ format: "png", size: 512 })}`));
            let json = await res.json();
            let attachment = new Discord.MessageAttachment(json.message, "whowouldwin.png");
